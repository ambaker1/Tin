...
Entries can be removed from the Tin List in a session with the command \cmdlink{tin remove}. 
\begin{syntax}
...
The command \cmdlink{tin clear} removes all entries from the Tin List, same as \textit{tin remove -all}.
\begin{syntax}
\command{tin clear}
\end{syntax}
...
By default, Tin auto-fetches on the fly when it cannot find Tin entries satisfying requirements. This can be toggled off with \cmdlink{tin auto}, which returns the current auto-fetch status.
...
The available packages in the Tin List can be queried with the command \cmdlink{tin packages}, and the existence of a package in the Tin List can be queried with the command \cmdlink{tin exists}.
\begin{syntax}
\command{tin packages} <-all | -tin | -auto> <\$pattern>
\end{syntax}
\begin{syntax}
\command{tin exists} <-all | -tin | -auto> \$name
\end{syntax}
\begin{args}
...
For each package in the Tin List, the command \cmdlink{tin get} can be used to query basic information about Tin.
...
\begin{syntax}
\command{tin check} -all <\$names> \\
tin check \$name <{}<-exact> \$version> <\$reqs ...>
\end{syntax}
\begin{syntax}
\command{tin upgrade} -all <\$names> \\
