...
\begin{syntax}
\command{tin assert} \$expr <\$op \$expected> <\$message>
\end{syntax}
\begin{args}
\$expr & Expression to evaluate. \\
\$op & Comparison operator, or ``is'' to assert type. Default ``is''. \\
\$expected & Comparison value. Default ``true''. \\
...
assertion error
    while executing
"tin assert {"hello world"} is integer"
\end{lstlisting}
\end{example}
