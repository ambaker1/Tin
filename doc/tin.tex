...
\clearpage
\section{Tin Entries}
The command \cmdlink{tin add} adds package installation instructions to the ``Tin''.
\begin{syntax}
\command{tin add} <-tin> \$name \$version \$repo \$tag \$file
\end{syntax}
\begin{args}
-tin & Option to add an entry to the Tin. Default. \\
\$name & Package name. \\
...
\end{args}
\begin{example}{Adding entries to the Tin}
\begin{lstlisting}
...
\clearpage
\section{Auto-Tin Entries}
To streamline the process of populating the Tin, simply use the command \cmdlink{tin add -auto} to use GitHub release tags to automatically add installation instructions.  
\begin{syntax}
\command{tin add -auto} \$name \$repo \$file <{}<-exact> \$version> <\$reqs ...>
\end{syntax}
\begin{args}
-auto & Option to add an auto-configuration to the Tin. \\
\$name & Package name. \\
\$repo & Github repository URL. \\
\$file & Installer file path in repo. \\
...
If a package is configured as an Auto-Tin package, the Tin can be automatically populated with versions available for installation with the command \cmdlink{tin fetch}. 
...
Both Tin entries and Auto-Tin configurations can be removed from the Tin in a session with the command \cmdlink{tin remove}. 
\begin{syntax}
